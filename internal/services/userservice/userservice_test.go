...
func TestUserService_Create(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			db.On("Create", mock.AnythingOfType("*model.User")).
				Return(&gorm.DB{
					Error: func() error {
						if tt.wantErr {
...
func TestUserService_GetByUsername(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			db.Mock.ExpectedCalls = nil
			db.On("First", mock.AnythingOfType("*model.User")).
				Return(&gorm.DB{
					Error: func() error {
						if tt.wantErr {
							return errors.New("error")
...
func TestUserService_Get(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			fields: func() fields {
				db := &mocks.DatabaseInterface{}
				db.On("First", mock.AnythingOfType("*model.User"), uint(1)).
					Return(&gorm.DB{
						Error: nil,
					})
...
func TestUserService_Update(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			fields: func() fields {
				db := &mocks.DatabaseInterface{}
				db.On("Save", mock.AnythingOfType("*model.User")).
					Return(&gorm.DB{
						Error: nil,
					})
				return fields{db}
...
func TestUserService_Delete(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			fields: func() fields {
				db := &mocks.DatabaseInterface{}
				db.On("Delete", mock.AnythingOfType("*model.User")).
					Return(&gorm.DB{
						Error: nil,
					})
				return fields{db}
